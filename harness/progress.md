### 2026-04-23T09:17:39+08:00 - MF-153 raw HTML style sanitizer verified

- Author: Codex Dispatcher with Researcher/Implementer/Reviewer subagents
- Focus: strict one-feature automation cycle after passing smoke; implemented one newly researched Typora parity item.
- Startup / baseline:
...
- Research updates:
  - Researcher used Typora Markdown Reference, HTML Support, Table Editing, Links, File Management, Images, and Export docs.
  - Appended `MF-153` for safe raw HTML inline style rendering and `MF-154` for visual table row/column drag reordering.
  - Dispatcher repaired the new entries to match the normalized harness schema by moving prose into `harness/features/MF-153.md` and `harness/features/MF-154.md`.
- Implemented feature work:
  - Selected `MF-153` because it was newly discovered, narrow, and terminal-verifiable.
  - Added a constrained `style` sanitizer for raw HTML decorations in `packages/editor/src/editor/decorations/inlineHtmlDecoration.ts`.
  - The sanitizer preserves only explicit safe CSS properties and values such as `color`, `background-color`, `font-weight`, `font-size`, `line-height`, `text-align`, `text-decoration`, and `vertical-align`.
  - Event handlers, unsupported attributes/properties, script/style tags, URL-bearing CSS, `expression(...)`, control characters, and dangerous CSS tokens remain stripped.
  - Added focused coverage in `packages/editor/src/editor/__tests__/inlineHtmlDecoration.test.ts`.
  - Promoted only `MF-153` to `status=verified`, `passes=true`, `lastVerifiedAt=2026-04-23T09:14:55+08:00`.
- Changed files:
...
- Simplifications made:
  - Kept style support to an explicit allowlist instead of general CSS pass-through.
  - Did not implement `MF-154` table drag handles or promote `MF-010`; `MF-010` remains `ready`, `passes=false`, and manual-gated.
- Verification:
  - Implementer passed focused inline HTML tests, editor lint, full editor test run, editor build, `pnpm harness:verify`, and `git diff --check` after reapplying the diff into the shared workspace.
  - Reviewer accepted the actual re-applied diff with no findings after read-only `git diff --check`, focused inline HTML tests, and `pnpm harness:verify`.
  - Dispatcher reran:
    - `pnpm --filter @markflow/editor exec vitest run src/editor/__tests__/inlineHtmlDecoration.test.ts` (`7` tests passed).
    ...
    - `pnpm harness:verify` (`154 total | verified=90 | ready=32 | planned=31 | blocked=1`; next remains `MF-010`).
    - `git diff --check`.
    - Final `./harness/init.sh --smoke`, which passed with desktop `84` tests and editor `505` passed / `3` skipped.
- Review:
  - Reviewer found the MF-153 promotion justified and scoped.
  - Residual risk: the CSS allowlist is intentionally narrow; broader Typora inline CSS remains unsupported until explicitly added.
- Next recommended feature:
  - `MF-010` remains harness-next but still needs live desktop/manual link acceptance before promotion.
  - If automation remains terminal-only, choose another small feature with complete automated coverage rather than promoting manual-gated items.
